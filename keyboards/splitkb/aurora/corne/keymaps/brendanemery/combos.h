...
enum combos { AT_SYMBOL, AMPERSAND, DOLLAR, TILDA, PERCENTAGE, HASH, COPY, CUT, PASTE, OPEN_BRACKET, CLOSE_BRACKET, OPEN_SQUARE_BRACKET, CLOSE_SQUARE_BRACKET, PLUS, UNDERSCORE, MINUS, UP_ARROW, ASTERISK, ESCAPE, SEMICOLON, HOME_NAV_MOUSE };
...
const uint16_t PROGMEM tilda_combo[]      = {LSFT_T(KC_H), KC_M, COMBO_END};
...
const uint16_t PROGMEM open_square_bracket_combo[]  = {KC_DOT, KC_EQL, COMBO_END};
const uint16_t PROGMEM close_square_bracket_combo[] = {KC_EQL, KC_MINS, COMBO_END};
...
combo_t key_combos[] = {
    [AT_SYMBOL] = COMBO(at_symbol_combo, KC_AT), [HASH] = COMBO(hash_combo, KC_HASH), [DOLLAR] = COMBO(dollar_combo, KC_DLR), [TILDA] = COMBO(tilda_combo, KC_GRV), [PERCENTAGE] = COMBO(percentage_combo, KC_PERC), [AMPERSAND] = COMBO(ampersand_combo, KC_AMPR), [OPEN_BRACKET] = COMBO(open_bracket_combo, KC_LPRN), [CLOSE_BRACKET] = COMBO(close_bracket_combo, KC_RPRN), [OPEN_SQUARE_BRACKET] = COMBO(open_square_bracket_combo, KC_LBRC), [CLOSE_SQUARE_BRACKET] = COMBO(close_square_bracket_combo, KC_RBRC), [PLUS] = COMBO(plus_combo, KC_PLUS), [UNDERSCORE] = COMBO(underscore_combo, KC_UNDS), [MINUS] = COMBO(minus_combo, KC_MINS), [ASTERISK] = COMBO(asterisk_combo, KC_ASTR), [ESCAPE] = COMBO(escape_combo, KC_ESC), [SEMICOLON] = COMBO(semicolon_combo, KC_SCLN), [COPY] = COMBO(copy_combo, KC_COPY), [CUT] = COMBO(cut_combo, KC_CUT), [PASTE] = COMBO(paste_combo, KC_PSTE),
